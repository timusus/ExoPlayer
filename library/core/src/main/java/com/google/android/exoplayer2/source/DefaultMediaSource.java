...
public final class DefaultMediaSource extends CompositeMediaSource<Void> {
  ...
  private long getRetryDelayMs(long loadDurationMs, IOException error, int errorCount) {
    return loadErrorHandlingPolicy == null
        ? Math.min((errorCount - 1) * 1000, 5000)
        : loadErrorHandlingPolicy.getRetryDelayMsFor(
            C.DATA_TYPE_UNKNOWN, loadDurationMs, error, errorCount);
  }
  ...
  private final class PeekLoadableCallback implements Loader.Callback<PeekLoadable> {
    ...
    public LoadErrorAction onLoadError(
        ...
        IOException error,
        int errorCount) {
      long retryDelayMs = getRetryDelayMs(loadDurationMs, error, errorCount);
